...
class SidebarItems<T extends Object> extends StatelessWidget {
  ...
  final void Function(T reorderedId, T droppedAt, DropAffinity affinity)? onReordered;
...
class _SidebarItemState<T extends Object> extends State<_SidebarItem<T>> {
  ...
  bool _onWillAccept(T? identifier, DropAffinity dropAffinity) {
    final accepted = widget.item.onWillAccept?.call(identifier, dropAffinity) ?? true;
  ...
  Widget _dropTarget({required bool renderDivider}) {
    return Container(
      ...
      child: renderDivider
          ? Center(
              child: Container(height: 2, color: MacosTheme.of(context).primaryColor),
            )
          : null,
    );
  }
  ...
  Widget build(BuildContext context) {
    ...
    final selectedColor = MacosDynamicColor.resolve(
      widget.item.selectedColor ?? _SidebarItemsConfiguration.of(context).selectedColor,
      context,
    );
    final unselectedColor = MacosDynamicColor.resolve(
      widget.item.unselectedColor ?? _SidebarItemsConfiguration.of(context).unselectedColor,
      context,
    );
        ...
        child: FocusableActionDetector(
          ...
          child: Container(
            ...
            decoration: ShapeDecoration(
              color: backgroundColor(),
              shape: widget.item.shape ?? _SidebarItemsConfiguration.of(context).shape,
            ),
                  ...
                  Padding(
                    ...
                    child: MacosIconTheme.merge(
                      data: MacosIconThemeData(
                        color: widget.selected ? MacosColors.white : MacosTheme.of(context).primaryColor,
                ...
                Expanded(
                  child: DefaultTextStyle(
                    ...
                    style: labelStyle.copyWith(
                      color: widget.selected || _isDraggingInside ? textLuminance(selectedColor) : null,
                    ),
    ...
    Widget? draggableSbItemWidget() {
      ...
      return DragTarget<T>(
          ...
          onMove: (details) {
            final shouldUpdate = _onWillAccept(details.data, DropAffinity.inside);
          ...
          builder: (context, accepted, rejected) {
            return Column(
              ...
              children: [
                if (renderDragTarget)
                  DragTarget<T>(
                    onWillAccept: (data) => _onWillAccept(data, DropAffinity.above),
                    onAccept: (data) => widget.onReordered!(
                      data,
                      widget.item.identifier,
                      DropAffinity.above,
                    ),
                    builder: (context, accepted, rejected) => _dropTarget(renderDivider: accepted.isNotEmpty),
                  ),
                renderDraggable
                    ? Draggable<T>(
                        ...
                        axis: Axis.vertical,
                        feedback: feedback,
                        childWhenDragging: Opacity(opacity: 0.5, child: baseSbItemWidget),
                        child: baseSbItemWidget,
                      )
                    : baseSbItemWidget,
                if (widget.isLastDisclousureItem == true && renderDragTarget)
                  DragTarget<T>(
                    onWillAccept: (data) => _onWillAccept(data, DropAffinity.below),
                    onAccept: (data) => widget.onReordered!(
                      data,
                      widget.item.identifier,
                      DropAffinity.below,
    ...
    final sbItemWidget = draggableSbItemWidget() ?? baseSbItemWidget;
...
class _DisclosureSidebarItem<T extends Object> extends StatefulWidget {
  ...
  @override
  __DisclosureSidebarItemState<T> createState() => __DisclosureSidebarItemState<T>();
}

class __DisclosureSidebarItemState<T extends Object> extends State<_DisclosureSidebarItem<T>>
        ...
        final item = _SidebarItem(
          ...
          item: SidebarItem(
            ...
            leading: Row(
              children: [
                GestureDetector(
                  onTap: () => widget.item.onExpanded(!widget.item.isExpanded),
                  child: Container(
                    color: Colors.transparent,
